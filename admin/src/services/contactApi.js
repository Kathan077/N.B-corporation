...
export const fetchContactContent = async () => {
  try {
    ...
    console.warn("Fetch Contact Content API Error:", error.message);
    return DEFAULT_CONTACT_CONTENT;
  }
};

export const saveFullContactContent = async (contentData) => {
  try {
    const res = await axios.post(CONTACT_CONTENT_API, contentData);
    return res.data;
  } catch (error) {
    console.error("Save Contact Content API Error:", error);
    throw error;
  }
};

export const resetContactContentToDefault = async () => {
  try {
    const res = await axios.post(`${CONTACT_CONTENT_API}/reset`);
    return res.data?.data || DEFAULT_CONTACT_CONTENT;
  } catch (error) {
    console.error("Reset Contact Content API Error:", error);
    throw error;
  }
};
...
export const fetchInquiries = async (params = {}) => {
  try {
    const res = await axios.get(INQUIRY_API, { params });
    if (res.data && Array.isArray(res.data.data)) {
      return res.data.data;
    }
    return [];
  } catch (error) {
    console.error("Fetch Inquiries API Error:", error.message);
    throw error;
  }
};
...
export const updateInquiry = async (id, updateData) => {
  try {
    const res = await axios.put(`${INQUIRY_API}/${id}`, updateData);
    return res.data;
  } catch (error) {
    console.error(`Update Inquiry [${id}] Error:`, error);
    throw error;
  }
};
...
export const deleteInquiry = async (id) => {
  try {
    const res = await axios.delete(`${INQUIRY_API}/${id}`);
    return res.data;
  } catch (error) {
    console.error(`Delete Inquiry [${id}] Error:`, error);
    throw error;
  }
};
