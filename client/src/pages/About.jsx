...
const resolveIcon = (name, Fallback = Award) => {
...
const About = () => {
  ...
  // Section configs
  const topBanner = aboutData?.topBanner || {
  ...
  const stats = (aboutData?.stats?.items || DEFAULT_STATS).filter(s => s.isActive !== false);
  ...
  const industries = (aboutData?.industries?.items || DEFAULT_INDUSTRIES).filter(i => i.isActive !== false);
  const categories = (aboutData?.categories?.items || DEFAULT_CATEGORIES).filter(c => c.isActive !== false);
  const pillars = (aboutData?.pillars?.items || DEFAULT_PILLARS).filter(p => p.isActive !== false);
  const values = (aboutData?.values?.items || DEFAULT_VALUES).filter(v => v.isActive !== false);
  ...
  return (
    <main ref={containerRef} className="bg-white font-sans selection:bg-brand-red selection:text-white overflow-hidden relative">
      ...
      {topBanner.isActive !== false && (
        <div className="bg-slate-950 text-white text-xs md:text-sm py-2.5 px-4 text-center font-mono border-b border-white/10 flex flex-wrap items-center justify-center gap-3 md:gap-8 pt-24 md:pt-28">
          <span className="flex items-center gap-1.5 text-brand-red font-bold">
            <ShieldCheck className="w-4 h-4" /> {topBanner.badge || 'AUTHORISED 3M DISTRIBUTOR'}
          </span>
          <span className="hidden md:inline text-gray-600">•</span>
          <span className="text-gray-300">{topBanner.tagline || 'ESTABLISHED IN 2006 (NARODA, AHMEDABAD)'}</span>
          <span className="hidden md:inline text-gray-600">•</span>
          <span className="text-amber-400 font-medium">{topBanner.motto || 'INNOVATION · INTEGRITY · EXCELLENCE'}</span>
        </div>
      )}
      ...
      {hero.isActive !== false && (
        <section className="relative w-full min-h-[85vh] py-12 lg:py-20 flex flex-col lg:flex-row items-center justify-between z-10">
          ...
          <div className="w-full lg:w-[50%] px-6 md:px-12 lg:px-20 h-full flex flex-col justify-center relative z-20">
            ...
            <div className="relative">
              ...
              <motion.div className="inline-flex items-center gap-2 bg-brand-soft border border-brand-red/20 px-3.5 py-1.5 rounded-full mb-6">
                ...
                <span className="text-xs md:text-sm font-bold text-brand-red tracking-wider uppercase">
                  {hero.badge || 'Excellence with Experience'}
                </span>
              </motion.div>
            ...
            >
              <a 
                href={hero.primaryCtaLink || '#brochure-overview'} 
              ...
              <a 
                href={hero.secondaryCtaLink || '#contact-info'} 
              ...
              >
                <span>{hero.secondaryCtaText || 'Contact Headquarters'}</span>
              </a>
            </motion.div>
          </div>
          ...
          <div className="w-full lg:w-[48%] mt-12 lg:mt-0 px-6 lg:px-12 relative z-10">
            <div className="relative w-full h-[420px] sm:h-[500px] lg:h-[580px] rounded-3xl overflow-hidden shadow-2xl border border-gray-200">
              <motion.img 
                ...
                src={hero.imageUrl || "https://images.pexels.com/photos/11479977/pexels-photo-11479977.jpeg"} 
              ...
              <div className="absolute top-6 right-6 bg-white/95 backdrop-blur-md p-4 rounded-2xl shadow-xl border border-white/40 max-w-[200px] text-center">
                <div className="text-[10px] font-bold tracking-widest text-gray-500 uppercase">{hero.sealTag || 'OFFICIAL PARTNER'}</div>
                <div className="text-xl font-black text-brand-red tracking-tight mt-0.5">{hero.sealTitle || '3M™ AUTHORISED'}</div>
                <div className="text-[11px] text-gray-700 font-medium">{hero.sealSubtitle || 'Science. Applied to Life.™'}</div>
              </div>
              ...
              <div className="absolute bottom-6 left-6 right-6 p-6 rounded-2xl bg-white/10 backdrop-blur-md border border-white/20 text-white">
                <div className="text-xs font-mono text-brand-red uppercase font-bold tracking-widest mb-1">{hero.profileTag || 'Company Profile'}</div>
                <h3 className="text-xl sm:text-2xl font-bold tracking-tight">{hero.profileHeading || 'Precision Converter & Manufacturer'}</h3>
                <p className="text-xs sm:text-sm text-gray-200 mt-1 line-clamp-2">
                  {hero.profileText || 'Delivering application-specific adhesive tapes, PU sealants, and industrial surface solutions across India.'}
                </p>
              </div>
            </div>
          </div>
      ...
      {/* Key Highlights Metrics Grid (Brochure Page 2) */}
      {aboutData?.stats?.isActive !== false && stats.length > 0 && (
        <section className="relative w-full py-12 lg:py-16 bg-slate-950 text-white border-y border-white/10 z-20">
          <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
              {stats.map((stat, idx) => {
                const Icon = resolveIcon(stat.icon, Award);
                return (
                  <motion.div 
                    key={stat.id || idx}
                    initial={{ opacity: 0, y: 30 }}
      ...
      {/* Corporate Overview & Story Section */}
      {story.isActive !== false && (
        <section className="relative w-full py-20 bg-white z-20">
          <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20">
            ...
            >
              <span className="text-xs sm:text-sm font-bold tracking-[0.4em] text-brand-red uppercase mb-3">
                {story.eyebrow || 'Company Overview'}
              </span>
              <h2 className="text-3xl sm:text-5xl md:text-6xl font-black text-gray-900 tracking-tight uppercase">
                {story.heading || 'Who We Are & What We Do'}
              </h2>
              ...
              >
                <div className="bg-brand-soft border-l-4 border-brand-red p-6 rounded-r-2xl">
                  <h3 className="text-xl font-bold text-gray-900 mb-2">
                    {story.storyTitle || 'Two Decades of Excellence (Est. 2006)'}
                  </h3>
                  ...
                  <div className="p-4 rounded-xl border border-gray-200 bg-gray-50 flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-900 text-sm">
                        {story.feature1Title || 'Custom Slitting & Converting'}
                      </h4>
                      <p className="text-xs text-gray-600">
                        {story.feature1Text || 'Tailored widths from 10mm to 1200mm master rolls.'}
                      </p>
                    </div>
                  </div>
                  <div className="p-4 rounded-xl border border-gray-200 bg-gray-50 flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="font-bold text-gray-900 text-sm">
                        {story.feature2Title || 'Pan-India Industrial Logistics'}
                      </h4>
                      <p className="text-xs text-gray-600">
                        {story.feature2Text || 'Rapid dispatch for zero production downtime.'}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              >
                ...
                <div className="bg-slate-900 text-white p-8 rounded-3xl relative overflow-hidden group shadow-xl">
                  ...
                  <div className="relative z-10">
                    ...
                    <h3 className="text-2xl sm:text-3xl font-black mb-3">{story.missionTitle || 'Our Mission'}</h3>
                ...
                <div className="bg-gradient-to-br from-brand-red to-brand-dark text-white p-8 rounded-3xl relative overflow-hidden group shadow-xl">
                  ...
                  <div className="relative z-10">
                    ...
                    <h3 className="text-2xl sm:text-3xl font-black mb-3">{story.visionTitle || 'Our Vision'}</h3>
      ...
      {aboutData?.industries?.isActive !== false && industries.length > 0 && (
        <section className="relative w-full py-20 bg-slate-950 text-white z-20 overflow-hidden">
          ...
          <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20 relative z-10">
            ...
            >
              <span className="text-xs sm:text-sm font-bold tracking-[0.4em] text-brand-red uppercase mb-3">
                {aboutData?.industries?.eyebrow || 'Comprehensive Applications'}
              </span>
              <h2 className="text-3xl sm:text-5xl md:text-6xl font-black tracking-tight uppercase">
                {aboutData?.industries?.heading || 'Industries We Power'}
              </h2>
              <p className="text-gray-400 text-base sm:text-lg max-w-2xl mx-auto mt-4">
                {aboutData?.industries?.subheading || 'Providing application-engineered 3M™ solutions tailored for critical sectors across manufacturing, safety, healthcare, and infrastructure.'}
              </p>
            </motion.div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
              {industries.map((ind, idx) => {
                const Icon = resolveIcon(ind.icon, Factory);
                return (
                  <motion.div
                    key={ind.id || idx}
                    initial={{ opacity: 0, y: 30 }}
                    ...
                    className={`p-6 sm:p-8 rounded-3xl bg-white/[0.02] border ${ind.borderColor || 'border-red-500/30'} hover:bg-white/[0.05] transition-all duration-300 group flex flex-col justify-between`}
                  >
                    <div>
      ...
      {aboutData?.categories?.isActive !== false && categories.length > 0 && (
        <section id="brochure-overview" className="relative w-full py-20 bg-gray-50 text-gray-900 z-20">
          <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20">
            ...
            >
              <span className="text-xs sm:text-sm font-bold tracking-[0.4em] text-brand-red uppercase mb-3">
                {aboutData?.categories?.eyebrow || 'Product Portfolio'}
              </span>
              <h2 className="text-3xl sm:text-5xl md:text-6xl font-black tracking-tight uppercase">
                {aboutData?.categories?.heading || '21 Core Product Categories'}
              </h2>
              <p className="text-gray-600 text-base sm:text-lg max-w-2xl mx-auto mt-3">
                {aboutData?.categories?.subheading || 'Detailed at a glance in the official NB Corporation product brochure.'}
              </p>
              ...
              <div className="mt-8 max-w-md mx-auto relative">
                ...
                {searchTerm && (
                  <button 
                    ...
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-gray-400 hover:text-gray-700 bg-gray-100 rounded-full w-5 h-5 flex items-center justify-center cursor-pointer"
                  >
                    ✕
                  </button>
                )}
      ...
      {aboutData?.pillars?.isActive !== false && pillars.length > 0 && (
        <section className="relative w-full py-20 bg-slate-950 text-white z-20">
          <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20">
            ...
            >
              <span className="text-xs sm:text-sm font-bold tracking-[0.4em] text-brand-red uppercase mb-3">
                {aboutData?.pillars?.eyebrow || 'Operating Standard'}
              </span>
              <h2 className="text-3xl sm:text-5xl md:text-6xl font-black tracking-tight uppercase">
                {aboutData?.pillars?.heading || 'Our Strategic Pillars'}
              </h2>
            </motion.div>
      ...
      {/* Core Values Section */}
      {aboutData?.values?.isActive !== false && values.length > 0 && (
          ...
          <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20">
            ...
            <div className="flex flex-col lg:flex-row gap-12 lg:gap-16 items-start">
              ...
              <div className="w-full lg:w-[35%]">
                <span className="text-xs sm:text-sm font-bold tracking-[0.4em] text-brand-red uppercase mb-3 block">
                  {aboutData?.values?.eyebrow || 'Foundation'}
                </span>
                <h2 className="text-4xl sm:text-6xl font-black tracking-tight uppercase text-gray-900">
                  {aboutData?.values?.heading || 'Our Core Values'}
                </h2>
                <p className="mt-6 text-gray-600 text-base leading-relaxed">
                  {aboutData?.values?.subheading || 'The uncompromising principles guiding NB Corporation since 2006 in delivering excellence, integrity, and innovative adhesive solutions.'}
                </p>
              </div>

              <div className="w-full lg:w-[65%] grid grid-cols-1 md:grid-cols-2 gap-6">
                {values.map((val, idx) => (
                  <div key={val.id || idx} className="p-8 rounded-3xl bg-gray-50 border border-gray-200 hover:shadow-lg transition-all">
                    <div className="text-3xl font-black text-brand-red font-mono mb-2">{val.num || `0${idx + 1}`}</div>
          ...
          <div className="max-w-7xl mx-auto px-6 md:px-12 lg:px-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
              ...
              <div className="p-8 rounded-3xl bg-white/[0.03] border border-white/10 flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-xs font-mono text-gray-400 uppercase tracking-widest mb-1">
                    {contact.addressTitle || 'Corporate Office & Warehouse'}
                  </h4>
              ...
              <div className="p-8 rounded-3xl bg-white/[0.03] border border-white/10 flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-xs font-mono text-gray-400 uppercase tracking-widest mb-1">
                    {contact.phonesTitle || 'Direct Hotlines'}
                  </h4>
              ...
              <div className="p-8 rounded-3xl bg-white/[0.03] border border-white/10 flex items-start gap-4">
                ...
                <div>
                  <h4 className="text-xs font-mono text-gray-400 uppercase tracking-widest mb-1">
                    {contact.emailTitle || 'Digital Correspondence'}
                  </h4>
                  <p className="text-sm font-bold text-white mb-2">{contact.email}</p>
                  {contact.website && (
                    <a 
                      href={contact.websiteUrl || `http://${contact.website}`} 
                      target="_blank" 
                      rel="noreferrer"
